...
async function seedDB() {
  ...
  const fs = require("fs");
  const data = fs.readFileSync("steamDBDetails_Cleaned.json");
    ...
    try {
      ...
      for (var key in json) {
        if (json.hasOwnProperty(key)) {
          ...
          if (json[key].success == true) {
            // console.log(json[key]);
            
            gameID = key;
            gameName = json[key].data.name;
            gameDescription = json[key].data.desc;
            
            if (gameDescription == null) {
              gameDescription = json[key].data.detailed_description;
            }

            if (json[key].data.is_free == true) {
              gamePrice = 0;
            } else {

                if (json[key].data.hasOwnProperty("price_overview")) {
                gamePrice = json[key].data.price_overview.final / 100;
              }
            }
            if (json[key].data.hasOwnProperty("categories")) {
              gameCategory = json[key].data.categories[0].description;
            } else {
              gameCategory = "Multi-player";
            }
            
            const categ = await Category.findOne({ title: gameCategory });
            

            gameImg = json[key].data.background_raw;
            gameReleaseDate = json[key].data.releaseDate;

            if (json[key].data.hasOwnProperty("developers")) {
              gameDeveloper = json[key].data.developers[0];
            } else {
              // gameDeveloper = "Unknown";
              gameDeveloper = faker.company.companyName(0)
            }

            if (json[key].data.hasOwnProperty("publishers")) {
              gamePublisher = json[key].data.publishers[0];
            } else {
