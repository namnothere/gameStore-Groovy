...
router.get("/", async (req, res) => {
  try {
    ...
    if (req.user) {
      let user = req.user;
      console.log(user);
      res.render("shop/home", { pageName: "Home", products: products, user: user});
    } else {
      res.render("shop/home", { pageName: "Home", products: products});
    }
...
router.post("/checkout", middleware.isLoggedIn, async (req, res) => {
  ...
  if (paymentMethod != null) {
    let credit = req.user.credit;
    if (credit < cart.totalCost) {
      req.flash("error", "Insufficient Balance");
      console.log("Insufficient Balance");
      return res.redirect("/user/profile");
    }

    // let just pretend uuid is order id
    // cuz idk how to generate from stripe module
    let paymentId = faker.datatype.uuid();
    ...
    order.save(async (err, newOrder) => {
      ...
      // reduce user credit
      // req.session.passport.user.credit -= cart.totalCost;
      // req.session.save(function(err) {console.log(err);});

      user.credit -= cart.totalCost;

      user.save(function(err) {
        if (err) return next(err)
        // What's happening in passport's session? Check a specific field...
        console.log("Before relogin: "+req.session.passport.user.credit)
    
        req.login(user, function(err) {
            if (err) return next(err)
    
            console.log("After relogin: "+req.session.passport.user.credit);
            // res.send(200);
          })
      })
